 ...
 * responses and converting raw sensor bytes into displayable pixel data.
 *
 * Kept free of any RPC/React/canvas dependency so it can be unit tested
 * directly (see test/frame.spec.ts). The generated proto module is a pure
 * data/enum definition file (no RPC framework dependency of its own), so
 * importing just the `PixelFormat` enum from it does not violate that.
 */
 ...
 * @param totalLength Total frame length in bytes (from CaptureFrameResponse.pixel_count).
