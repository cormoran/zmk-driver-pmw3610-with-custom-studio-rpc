 ...
 * return NULL on an out-of-range index. Only used below by the frame
 * handlers -- GetInfo/ReadDiagnostics/ReadRegister/WriteRegister moved to
 * pmw3610_request_exec.c (shared with the split relay peripheral
 * executor). */
...
static bool pmw3610_rpc_handle_request(const zmk_custom_CallRequest *raw_request,
    ...
    case cormoran_pmw3610_Request_write_register_tag: {
        ...
        if (source == 0) {
            pmw3610_request_exec_handle(&req, resp);
            break;
        }
#if IS_ENABLED(CONFIG_ZMK_PMW3610_SPLIT_RPC_RELAY)
