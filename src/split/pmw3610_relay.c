    ...
    *out_resp = (cormoran_pmw3610_RelayResponse)cormoran_pmw3610_RelayResponse_init_zero;
    ...
    if (!relay_req.has_request ||
        ...
        snprintf(err.message, sizeof(err.message),
                 "unsupported relayed pmw3610 request type (frame capture/streaming does not "
                 "support relaying yet)");
...
static bool encode_peripheral_response_notification_payload(pb_ostream_t *stream,
...
static int on_pmw3610_relay_response(const zmk_event_t *eh) {
    ...
    if (index < 0) {
        LOG_WRN("Received a pmw3610 relay response but the cormoran__pmw3610 Studio subsystem "
                "is not registered (CONFIG_ZMK_PMW3610_STUDIO_RPC not enabled?)");
        return 0;
    }
    ...
    pb_callback_t payload = {
        .funcs.encode = encode_peripheral_response_notification_payload,
        .arg = (void *)&peripheral_response_notification,
    };
...
static int pmw3610_split_relay_test_init(void) {
    ...
    /* Also exercise the "unsupported request kind" path (frame capture does
     * not support relaying yet), reusing the same request_id. */
    ...
    if (!pb_encode(&ostream, cormoran_pmw3610_RelayRequest_fields, &relay_req)) {
        LOG_ERR("Split relay test: failed to encode unsupported-kind request: %s",
                PB_GET_ERROR(&ostream));
        return -EIO;
    }
    ret = pmw3610_relay_exec_request(payload, ostream.bytes_written, &relay_resp);
    if (ret < 0) {
        LOG_ERR("Split relay test: unsupported-kind exec failed: %d", ret);
        return ret;
    }
    if (relay_resp.response.which_response_type != cormoran_pmw3610_Response_error_tag) {
        LOG_ERR("Split relay test: expected an ErrorResponse for an unsupported request kind, "
                "got response type %d",
                relay_resp.response.which_response_type);
        return -EINVAL;
    }
